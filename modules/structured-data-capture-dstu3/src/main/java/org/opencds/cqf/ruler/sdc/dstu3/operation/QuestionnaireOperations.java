package org.opencds.cqf.ruler.sdc.dstu3.operation;
...
import org.opencds.cqf.ruler.sdc.config.ExtractConfig;
...
import static org.opencds.cqf.ruler.common.helper.ClientHelper.getClient;

@Component
public class QuestionnaireOperations {

    private FhirContext fhirContext;
    private ExtractConfig extractConfig;

    @Inject
    public QuestionnaireOperations(FhirContext fhirContext, ExtractConfig extractConfig) {
    ...
    private Bundle sendObservationBundle(Bundle observationsBundle) throws IllegalArgumentException {
        String url = extractConfig.getQuestionnaireResponseExtractEndpoint();
        if (null == url || url.length() < 1) {
            throw new IllegalArgumentException(
                    "Unable to transmit observation bundle.  No observation.endpoint defined in hapi.properties.");
        }
        String user = extractConfig.getQuestionnaireResponseExtractUserName();
        String password = extractConfig.getQuestionnaireResponseExtractPassword();

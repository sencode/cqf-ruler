...
import org.opencds.cqf.ruler.sdc.config.TransformConfig;
...
public class ObservationOperations {
    ...
    @Inject
    public ObservationOperations(FhirContext fhirContext, TransformConfig transformConfig){
    ...
    public Bundle transformObservations(
        ...
        IGenericClient client = getClient(fhirContext, conceptMapURL, transformConfig.getObservationTransformUsername(), transformConfig.getObservationTransformPassword() );
                ...
                if(observation.getValue().fhirType().equalsIgnoreCase("codeableconcept")){
                    ...
                    if(obsValueCode != null) {
                        if (codeMappings.get(observation.getValueCodeableConcept().getCoding().get(0).getCode()) != null) {
                            if (transformConfig.getObservationTransformReplaceCode()) {
